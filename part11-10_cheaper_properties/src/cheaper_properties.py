...
def cheaper_properties(properties: list, reference: RealProperty):
    # return a list containing only properties in the original list which are cheaper than the reference property
    # return (property, price difference)
    return [(property, property.price_difference(reference)) for property in properties if not property.more_expensive(reference) and not property == reference]
