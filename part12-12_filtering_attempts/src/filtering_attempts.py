...
def attempts_with_grade(attempts: list, grade: int):
    return list(filter(lambda n: n.grade == grade, attempts))

def passed_students(attempts: list, course: str):
    grade_filter = list(filter(lambda k: k.grade > 0, attempts))
    course_filter = list(filter(lambda t: t.course_name == course, grade_filter))
    return sorted(list(map(lambda m: m.student_name, course_filter)))
